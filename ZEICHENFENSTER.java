...
public class ZEICHENFENSTER
{
    ...
    private Image canvasImage, k;
    private int pos_x, pos_y;
    private boolean  pressed,restart;
    ...
    {
        this(titel, 640, 640, Color.white); 
    ...
    public void zeichneKreisg(int x, int y, int radius)
    {
    ...
    public void zeichneKreisc(int x, int y, int radius)
    {
    ...
    public void fuelleRechteck2(int xPos, int yPos, int breite, int hoehe, Color farbe)
    {
        Color original=graphic.getColor();
    ...
    {
        ZEICHENFENSTER.gibFenster().zeichneBild(k,0,0);
        //Color original = graphic.getColor();
        //graphic.setColor(backgroundColor);
        //Dimension size = canvas.getSize();
    ...
    public void zeichneTextb(String text, int x, int y)
    {
        graphic.setColor(Color.blue);
        graphic.drawString(text, x, y);   
        canvas.repaint();        
    ...
    public void zeichneZahl(long zahl, int x, int y)
    {
        graphic.setColor(Color.black);
        graphic.drawString(String.valueOf(zahl), x, y);   
        canvas.repaint();
    }
    
    public void zeichneZahlb(long zahl, int x, int y)
    {
        graphic.setColor(Color.red);
        graphic.setFont(graphic.getFont().deriveFont(50F));
        graphic.drawString(String.valueOf(zahl)+"km", x, y);
        canvas.repaint();
        graphic.setColor(Color.black);
    }
    ...
    public void loescheZahlt(long zahl, int x, int y)
    {
        Color original = graphic.getColor();
        graphic.setColor(backgroundColor);
        graphic.drawString(String.valueOf(zahl), x, y);   
